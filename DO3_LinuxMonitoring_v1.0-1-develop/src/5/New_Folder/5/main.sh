...
top_5_folders=$(du -h --max-depth=1 "$directory" | sort -hr | head -5 | awk '{print $2, $1}' | nl -w1 -s " - /" | sed 's/$/,/')
...
text_files=$(find "$directory" -type f | grep -E ".txt$" | wc -l)
...
top_10_files=$(find "$directory" -type f -exec du -h {} + | sort -hr | head -10 | awk '{
    file=$2; size=$1;
    n = split(file, parts, ".");
...
top_10_exec_files_md5=$(find "$directory" -type f -executable -exec du -h {} + | sort -hr | head -10 | awk '{print $2}' | xargs -I {} sh -c 'echo "$(du -h {} | cut -f1) $(md5sum {} | cut -d" " -f1) {}"' | sort -hr | nl -w1 -s " - /" | sed 's/$/,/' | awk '{print $0, "exe"}' | sed 's/.*/&,/' )
