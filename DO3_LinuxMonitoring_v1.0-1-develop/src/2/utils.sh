...
calculate_uptime() {
    ...
    info+="Uptime (seconds): $uptime_sec\n"
}

ask_to_save_log() {
    local info=$1
    read -p "Вы хотите записать эту информацию в файл журнала? (yes/no) " answer

    if [[ $answer =~ ^[Yy] ]]; then
        logfile="$(date +"%d_%m_%y_%H_%M_%S").txt"
        echo -e "$info" >> "$logfile"
        echo "Информация записана в лог '$logfile'."
    elif [[ $answer =~ ^[Nn] ]]; then
        echo "Отмена записи"
    else 
        echo "Использование [Yy] или [Nn]"
    fi        
}
